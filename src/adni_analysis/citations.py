...
def _http_get(url: str, *, timeout_s: float = 30.0) -> bytes:
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "adni-analysis/0.1 (contact: unknown@example.com)",
            "Accept": "*/*",
        },
    )
...
def format_ama_journal_reference(article: PubMedArticle) -> str:
    """
    Format a PubMedArticle as an AMA-style journal reference suitable for JAMA Network submissions.
    """
    ...
    if article.doi:
        doi_part = f"doi:{article.doi}."
        parts.append(doi_part)

    return " ".join([p for p in parts if p])


def format_nature_journal_reference(article: PubMedArticle) -> str:
    """
    Format a PubMedArticle in a Nature Portfolio–style reference string (npj series).

    Notes:
    - Nature style uses numbered references.
    - All authors are listed unless there are more than 5, in which case the first author is listed followed by 'et al.'.
    - Journal abbreviation is taken from PubMed ISOAbbreviation.
    """

    def fmt_initials(initials: str) -> str:
        initials = initials.strip().replace(".", "")
        if not initials:
            return ""
        # Insert periods after each initial (e.g., "DH" -> "D. H.").
        return " ".join([f"{ch}." for ch in initials if ch.isalnum()])

    def fmt_author(a: PubMedAuthor) -> str:
        if a.collective_name:
            return _collapse_ws(a.collective_name)
        last = (a.last_name or "").strip()
        ini = fmt_initials(a.initials or "")
        if last and ini:
            return f"{last}, {ini}"
        if last:
            return last
    ...
    title = article.title.rstrip(".") + "."
    journal = _collapse_ws(article.journal_abbrev or "")
    ...
    tail = journal
    if vol:
        tail = (tail + " " + vol) if tail else vol
    if pages:
        tail = (tail + ", " + pages) if tail else pages
    if article.doi:
        tail = (tail + f"; {article.doi}") if tail else str(article.doi)
    if year:
        tail = (tail + f" ({year})") if tail else f"({year})"
